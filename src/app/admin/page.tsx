...
interface Activity {
  id: string
  type: 'order' | 'user' | 'product' | 'support'
  message: string
  time: string
  user: string
  icon: string
...
export default function AdminDashboardPage() {
  ...
  const [topProducts, setTopProducts] = useState<TopProduct[]>([
    ...
    { id: '5', name: 'TOEFL Practice Tests', sales: 87, revenue: 7743, growth: 8 },
  ])

  const [activities, setActivities] = useState<Activity[]>([
    { id: '1', type: 'order', message: 'New order #ORD-2024-006 received', time: '5 minutes ago', user: 'John Doe', icon: '🛒' },
    { id: '2', type: 'user', message: 'New user registered: Emily Brown', time: '15 minutes ago', user: 'Emily Brown', icon: '👤' },
    { id: '3', type: 'product', message: 'Product "TOEFL Practice Tests" updated', time: '1 hour ago', user: 'Admin', icon: '📦' },
    { id: '4', type: 'support', message: 'New support ticket #TKT-2024-007', time: '2 hours ago', user: 'Sarah Chen', icon: '🎫' },
    { id: '5', type: 'order', message: 'Order #ORD-2024-003 marked as completed', time: '3 hours ago', user: 'System', icon: '🛒' },
    { id: '6', type: 'product', message: 'Low stock alert: "GRE Course" (3 left)', time: '5 hours ago', user: 'System', icon: '⚠️' },
  ])
  ...
  const getActivityColor = (type: string) => {
    switch(type) {
      ...
      case 'support': return 'from-yellow-500 to-orange-500'
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <div className="flex items-center gap-2 mt-2">
                <span className="text-green-400 text-sm font-medium">↑ {stats.revenueGrowth}%</span>
        ...
        <div className="bg-gradient-to-br from-blue-600/20 to-cyan-600/20 rounded-xl p-6 border border-white/20 hover:scale-105 transition-all">
          <div className="flex justify-between items-start">
            <div>
              ...
              <div className="flex items-center gap-2 mt-2">
                <span className="text-blue-400 text-sm font-medium">↑ {stats.ordersGrowth}%</span>
                <span className="text-xs text-gray-500">vs last month</span>
              </div>
              <p className="text-xs text-yellow-400 mt-1">{stats.pendingOrders} pending</p>
            </div>
        ...
        <div className="bg-gradient-to-br from-purple-600/20 to-pink-600/20 rounded-xl p-6 border border-white/20 hover:scale-105 transition-all">
          <div className="flex justify-between items-start">
            <div>
              ...
              <div className="flex items-center gap-2 mt-2">
                <span className="text-purple-400 text-sm font-medium">↑ {stats.usersGrowth}%</span>
                <span className="text-xs text-gray-500">vs last month</span>
              </div>
              <p className="text-xs text-green-400 mt-1">+{stats.newUsers} this week</p>
            </div>
        ...
        <div className="bg-gradient-to-br from-yellow-600/20 to-orange-600/20 rounded-xl p-6 border border-white/20 hover:scale-105 transition-all">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-xs text-red-400 mt-2">{stats.lowStock} low in stock</p>
              <p className="text-xs text-gray-400 mt-1">Conv. rate: {stats.conversionRate}%</p>
            </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white/5 backdrop-blur-sm rounded-xl border border-white/20 overflow-hidden">
          <div className="p-4 border-b border-white/10 flex justify-between items-center">
            ...
            <Link href="/admin/orders" className="text-sm text-pink-400 hover:text-pink-300">
              View All →
            </Link>
          </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white/5 backdrop-blur-sm rounded-xl border border-white/20 p-6">
          ...
          <div className="space-y-3">
            ...
            <Link
              href="/admin/support"
              className="flex items-center gap-3 p-3 bg-gradient-to-r from-red-600/20 to-pink-600/20 rounded-lg hover:scale-105 transition-all border border-white/10"
            >
              <span className="w-8 h-8 bg-red-600/30 rounded-lg flex items-center justify-center text-xl">🎫</span>
              <div>
                <p className="text-white font-medium">Support Tickets</p>
                <p className="text-xs text-gray-400">Manage customer support</p>
              </div>
            </Link>
          </div>
        </div>
